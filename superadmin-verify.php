...
$User = new UserModel();
$security = new LoginSecurityService($User->getDb());
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals((string) ($_SESSION['csrf_token'] ?? ''), $csrf)) {
        ...
        $security->event((int) $pending['user_id'], 'superadmin_2fa_expired', '2FA kodu süresi veya deneme limiti aşıldı.');
        unset($_SESSION['pending_superadmin_2fa']);
        $error = 'Kodun süresi doldu. Yeniden giriş yapın.';
    } elseif (!password_verify($code, (string) $pending['code_hash'])) {
        $_SESSION['pending_superadmin_2fa']['attempts']++;
        $pending = $_SESSION['pending_superadmin_2fa'];
        $security->event((int) $pending['user_id'], 'superadmin_2fa_failed', 'Hatalı 2FA kodu girildi.');
        ...
        if (!$user || (int) ($user->superadmin ?? 0) !== 1 || (int) $user->status !== 1) {
            ...
            $security->event((int) $user->id, 'superadmin_login_success', 'Superadmin 2FA ile giriş yaptı.');
...
<head>
  ...
  <title>İki Aşamalı Doğrulama | Puantor</title>
  ...
  <style>
    ...
    .bg-blob-primary {
      width: 350px;
      height: 350px;
      background: radial-gradient(circle, rgba(32, 107, 196, .2) 0%, rgba(32, 107, 196, 0) 70%);
      top: -100px;
      left: -100px;
    }
    ...
    .bg-blob-indigo {
      width: 350px;
      height: 350px;
      background: radial-gradient(circle, rgba(99, 102, 241, .15) 0%, rgba(99, 102, 241, 0) 70%);
      bottom: -100px;
      right: -100px;
    }
    ...
    .login-container {
      ...
      box-shadow: 0 20px 40px -15px rgba(0, 0, 0, .05), 0 1px 3px rgba(0, 0, 0, .02);
      z-index: 10;
      position: relative;
    ...
    .otp-input {
      ...
      text-align: center;
      padding: 0;
      caret-color: var(--mobile-primary);
    ...
    .btn-primary {
      ...
      border: none;
      box-shadow: 0 8px 24px rgba(32, 107, 196, .2);
      transition: all .25s ease;
    }

    .btn-primary:hover {
      background: linear-gradient(135deg, #2475d7 0%, #1d61b3 100%);
      box-shadow: 0 10px 28px rgba(32, 107, 196, .3);
      transform: translateY(-1px);
    }

    .btn-primary:active {
      transform: translateY(1px) scale(.98);
      box-shadow: 0 4px 12px rgba(32, 107, 196, .15);
    }
...
<body>
  ...
  <div class="login-container">
    <div class="text-center mb-4">
      <div class="mb-3 d-flex justify-content-center align-items-center" style="height: 80px; filter: drop-shadow(0 4px 12px rgba(0,0,0,.03));">
        <?php
      ...
      <span class="verification-icon"><i class="ti ti-shield-lock"></i></span>
      <h1 class="verification-title">İki Aşamalı Doğrulama</h1>
      <p class="login-subtitle">E-posta adresinize gönderilen 6 haneli doğrulama kodunu girin. Kod 5 dakika geçerlidir.</p>
    </div>

    <?php if ($error): ?>
      <div class="alert alert-danger d-flex align-items-center gap-2 py-2 px-3 mb-3 animate-fade-in" style="border-radius:14px;background-color:#fef2f2;border:1px solid #fca5a5;color:#991b1b;">
    ...
    <form method="post" autocomplete="off">
      ...
      <div class="mb-3">
        <label for="otpDigit1" class="form-label text-secondary fw-medium">Doğrulama kodu</label>
        <input type="hidden" id="verificationCode" name="code">
        <div class="otp-group" id="otpGroup" role="group" aria-label="6 haneli doğrulama kodu" aria-describedby="codeHelp">
          <?php for ($digit = 1; $digit <= 6; $digit++): ?>
            <input
              type="text"
              id="otpDigit<?php echo $digit; ?>"
              class="otp-input"
              inputmode="numeric"
              pattern="[0-9]"
              maxlength="1"
              aria-label="<?php echo $digit; ?>. hane"
              <?php echo $digit === 1 ? 'autocomplete="one-time-code" autofocus' : 'autocomplete="off"'; ?>
              required
            >
          <?php endfor; ?>
        </div>
        <div id="codeHelp" class="form-hint text-center mt-2">Kod yalnızca rakamlardan oluşur.</div>
      </div>
      <button class="btn btn-primary w-100 mb-3" type="submit">
        <i class="ti ti-shield-check me-2"></i>Doğrula ve Giriş Yap
      </button>
    </form>

    <div class="text-center">
      <a href="sign-in.php" class="text-secondary text-decoration-none hover-underline">
        <i class="ti ti-arrow-left me-1"></i>Giriş ekranına dön
      </a>
    </div>
  </div>
  ...
  <script>
    ...
    function fillOtp(value) {
      ...
      otpInputs[Math.min(digits.length, otpInputs.length - 1)].select();
    }
