...
$duyurular = [];
try {
    $duyurular = $Duyuru->getBildirimler($kullanici_id, $firma_id, $is_superadmin, $is_main_user);
} catch (Exception $e) {
...
<div class="container px-3">
  ...
  <?php if (empty($duyurular)): ?>
    ...
    <div class="mobile-card text-center py-5 px-3 d-flex flex-column align-items-center justify-content-center" style="border-radius: 20px; background: #fff; min-height: 250px;">
      ...
      <p class="text-muted text-xs mb-0 px-4">Herhangi bir yeni bildirim aldığınızda burada listelenecektir.</p>
    </div>
  <?php else: ?>
    <!-- Bildirim Listesi -->
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '../pages/duyurular/api.php',
                    method: 'POST',
                    data: { action: 'tumunu_okundu', type: 'bildirim' },
                    dataType: 'json',
                    success: function(res) {
                        if (res.success) {
                            Swal.fire({
